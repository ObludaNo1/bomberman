...
fn fragment(input: VertexOutput) -> @location(0) vec4<f32> {
    var texture_read = textureSample(tileset_texture, tileset_texture_sampler, input.uv);
    var texture_r = texture_read.r;

    var result_colour: vec4<f32>;
    if (texture_r <= BLACK_THRESHOLD) {
        result_colour = colour_1;
    } else if (texture_r <= DARK_THRESHOLD) {
        result_colour = colour_2;
    } else if (texture_r <= LIGHT_THRESHOLD) {
        result_colour = colour_3;
    } else {
        result_colour = colour_4;
    }

    result_colour.a = result_colour.a * texture_read.a;
